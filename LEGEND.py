...
async def help_command(update: Update, context: CallbackContext):
    ...
    help_message = (
        ...
        "*6. /help* - Show this help message with a list of available commands.\n"
    )
...
async def generate_redeem_code(user_name, duration_hours=1):
    ...
    # Store the expiry time (1 hour later for trials)
...
async def redeem_access(update: Update, context: CallbackContext):
    ...
    redeem_codes[code] = expiry_time.isoformat()  # Store as ISO format string
...
async def redeem_code(update: Update, context: CallbackContext):
    ...
    expiry_time = datetime.fromisoformat(redeem_codes.pop(code))  # Convert back to datetime
    user_access[user_id] = expiry_time.isoformat()  # Store as ISO format string
...
async def has_access(user_id):
    ...
    return datetime.fromisoformat(user_access.get(user_id, datetime.min.isoformat())) > current_time
...
async def trail(update: Update, context: CallbackContext):
    ...
    # Notify the admin about the trail request
    trial_requests[user_id] = user_name  # Store the request with user_id as the key
    save_data()
...
async def approve_trial(update: Update, context: CallbackContext):
    ...
    user_name = trial_requests.pop(user_id, None)  # Removes their request and gets the username, if exists
    ...
    # Optional: Log event
    print(f"Trial approved for {user_id} ({user_name})")
...
async def disapprove_trial(update: Update, context: CallbackContext):
    ...
    message = (
        f"Sorry, you cannot get a trial. Please contact: @Rohan2349"
    )
    ...
    # Optional: Log event
    print(f"Trial disapproved for {user_id} ({user_name})")
...
def main():
    ...
    application.add_handler(CommandHandler("attack", attack))  # Attack command
    application.add_handler(CommandHandler("trail", trail))  # Trail request command
