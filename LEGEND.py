...
async def generate_redeem_code(user_name, duration):
...
async def redeem_access(update: Update, context: CallbackContext):
    chat_id = update.effective_chat.id
    user_id = str(update.effective_user.id)  # Using string for key

    # Check if the message sender is the admin
    if user_id != ADMIN_ID:
        await context.bot.send_message(chat_id=chat_id, text="*❌ You are not authorized to redeem access!*", parse_mode='Markdown')
        return
    ...
    code = await generate_redeem_code(user_name, duration)
    ...
    await context.bot.send_message(chat_id=chat_id, text=f"*✅ Redemption code generated: {code}*\n*This code grants access for {duration} days!*\n*Users can use /redeem_code <code>*", parse_mode='Markdown')
...
async def redeem_code(update: Update, context: CallbackContext):
    ...
    expiry_time = redeem_codes.pop(code)  # Remove code after redeeming
